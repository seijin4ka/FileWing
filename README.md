...
FileWingは3つの認証方式をサポートしています。`AUTH_METHOD`環境変数で切り替え可能です。

| AUTH_METHOD | 説明 | ログアウト | 推奨用途 |
|-------------|------|-----------|---------|
| `local` | ローカル認証（管理者登録方式） | アプリ内で可能 | SSOを使わずすぐに使い始めたい場合（既定値） |
...
`AUTH_METHOD = "local"` が既定値です。IdPの設定なしでそのまま使い始められます。

1. デプロイ後、公開URLに最初にアクセスすると**管理者登録画面**が表示される
...
`wrangler.toml` は `AUTH_METHOD = "local"`（管理者登録方式）を既定値としています。
デプロイ後、公開URLに最初にアクセスすると管理者登録画面が表示されるので、
その場で管理者アカウントを作成すればすぐに使い始められます。
管理者登録が済むまで、また未ログインの状態ではアプリの機能にはアクセスできません。

SAML SSOやCloudflare Accessを使う場合は下記「認証設定」を参照してください。
...
| 変数名 | 必須 | 説明 | 例 |
|-------|:----:|------|-----|
| `AUTH_METHOD` | - | 認証方式（省略時は自動判定、判定できない場合は`local`） | `local`, `saml`, `cloudflare-access`, `skip` |

...
デプロイ後は公開URLにアクセスして、画面の案内どおりに認証方式を選ぶだけで使い始められます。
...
FileWingは3つの認証方式をサポートしています。
**通常は初期セットアップ画面から選択します。** `AUTH_METHOD`環境変数で固定することもできます
（環境変数を設定した場合はそちらが優先されます）。

| AUTH_METHOD | 説明 | ログアウト | 推奨用途 |
|-------------|------|-----------|---------|
| `local` | ローカル認証（管理者登録方式） | アプリ内で可能 | SSOを使わずすぐに使い始めたい場合 |
...
初期セットアップ画面で「管理者アカウントを作成」を選ぶとこの方式になります。
IdPの設定なしでそのまま使い始められます。

1. 初期セットアップ画面で「管理者アカウントを作成」を選択すると**管理者登録画面**が表示される
2. メールアドレスとパスワード（12文字以上）を入力して管理者アカウントを作成
3. 登録と同時にログイン状態になり、そのまま利用できる
4. 管理者が登録された後は登録画面が閉じ、以降はログイン画面が表示される

パスワードはPBKDF2（SHA-256、10万回、ソルト付き）でハッシュ化して保存し、
ログイン試行にはブルートフォース対策のレート制限（5回失敗で30分ロックアウト）が適用されます。
セッション署名キーは `SESSION_SECRET` が未設定の場合、初回アクセス時に自動生成してD1に保存します。

> **注意**: 最初にアクセスした人が管理者になります。
> デプロイ後は速やかに管理者登録を済ませてから、公開URLを共有してください。
...
**デプロイ時にシークレットの入力を求められることはありません。**
セッション署名キーは自動生成され、SAML証明書は
SAMLを使う場合のみ初期セットアップ画面で入力します。

- **データベースのテーブルは初回アクセス時に自動作成されます。**
  デプロイコマンドで `wrangler d1 migrations apply` を実行する必要はありません
- **認証方式は初期セットアップ画面で選択します**（次項）

### 初期セットアップ

デプロイ後、公開URLに最初にアクセスすると**初期セットアップ画面**が表示され、
認証方式を選択できます。

| 選択肢 | 内容 |
|--------|------|
| **管理者アカウントを作成** | メールアドレスとパスワードで管理者アカウントを作成します。IdPの準備が不要で、そのまま使い始められます |
| **SAML SSOを設定** | IdPのSSO URL・Entity ID・X.509証明書を画面から入力します。Google Workspaceなどの既存アカウントでログインできるようになります |

選択した内容はD1に保存されるため、`wrangler.toml` の編集や
`wrangler secret put` は不要です。

- SAMLを選んだ場合、SP Entity ID と ACS URL はアクセス中のURLから自動で入力されます。
  この値をIdP側のSAMLアプリに登録してください
- SAMLを選んだ場合、最初にSSOでログインしたユーザーが管理者になります
- セットアップ完了後はセットアップ画面と管理者登録画面が閉じ、
  以降はログイン画面のみが表示されます

> **注意**: 最初にアクセスした人がセットアップを実行できます。
> デプロイ後は速やかにセットアップを完了してから、URLを共有してください。

環境変数で `AUTH_METHOD` などを明示的に設定した場合は、
そちらが画面での設定より優先されます。
...
初回デプロイ時にD1データベースとR2バケットが自動作成され、
バインディングに紐付けられます。テーブルは初回アクセス時に
アプリ自身が作成するため、マイグレーションの実行は不要です。
...
> デプロイコマンドは `npx wrangler deploy` のままで問題ありません。
> スキーマ作成と認証設定はアプリ側で完結します。

#### デプロイ後の設定（任意）

認証は初期セットアップ画面で設定できるため、通常は追加の設定は不要です。
以下は必要に応じて `wrangler.toml` の `[vars]` で設定します。

| 変数 | 説明 |
|------|------|
| `EMAIL_FROM` | Email Routingで検証済みの送信元アドレス（メール通知を使う場合は必須） |
| `APP_URL` | ログアウト後のリダイレクト先 |
| `AUTH_METHOD` | 認証方式を環境変数で固定する場合。画面での設定より優先されます |

シークレットは事前設定が不要です。

| シークレット | 扱い |
|-------------|------|
| `SESSION_SECRET` | 未設定の場合は初回アクセス時に自動生成してD1に保存します |
| `SAML_IDP_CERT` | 初期セットアップ画面で入力してD1に保存します（SAMLを使う場合のみ） |

固定値を使いたい場合のみ、以下で設定してください。
...
| 変数名 | 必須 | 説明 | 例 |
|-------|:----:|------|-----|
| `AUTH_METHOD` | - | 認証方式（省略時は初期セットアップ画面で選択） | `local`, `saml`, `cloudflare-access`, `skip` |

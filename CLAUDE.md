...
### リンク管理
- 全リンク一覧表示（`/links`）
- 統計表示（総リンク数、有効リンク数、総ダウンロード数）
- リンク詳細モーダル（ダウンロード統計、送信先、履歴）
- リンクのコピー、無効化操作
...
### ファイル削除時の連鎖削除
- ファイル削除時、関連するリンクをD1から完全削除（無効化ではなく物理削除）
- 関連データ（link_recipients, download_logs）も同時削除
- D1の従量課金対策としてレコード数を最小化
...
- 孤児リンクを削除（削除済みファイルに紐づくリンク・送信先履歴・ダウンロード履歴）
- レート制限の古いレコードを削除
